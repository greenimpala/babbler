...
var updateProfilePictures = function(accessToken, userId, callback) {
	...
	getBigProfilePictureImagePath(accessToken, function (err, fullURL) {
		if (err) { return callback(err); }
		
		getProfilePicAndUploadToS3(fullURL, userId, function (err, pictureURL) {
	...
	http.get({
		...
		res.on('end', function (err) {
			...
			return models.User.findOne({ profile_id: userId }, function (err, user) {
				...
				console.log("Uploading picture to AmazonS3");
