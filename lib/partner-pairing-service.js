	...
	setInterval(function () {
		...
		for (i = 0; i < length; i += 1) {
			...
			if (socket_one.handshake.fb_user._id.toString() === 
				...
				// TODO: Inform socket they are not searching
				continue;
			}

			// Check if partners are already friends
			models.ChatSession.findOne({ $and : [
				{ 'participants._id' : socket_one.handshake.fb_user._id },
				{ 'participants._id' : socket_two.handshake.fb_user._id }
			]}, function (err, session) {
				if (session) {
					var are_friends = true;
				}
